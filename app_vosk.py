...
def background_listener():
    # Load Vosk model
    asr_model = Model("model/vosk-model-small-en-us-0.15")
    ...
    log_to_memory("Listening... (press Ctrl+C to stop)")
    try:
        while True:
...
class VoiceAssistant:
    def __init__(self, log_to_memory, command_registry, command_queue):
    ...
    def try_to_recognize(self, repeated=False):
        ...
        if not repeated:
            ...
            self.log_to_memory("Hello, you have called for me. How can i help you?")
        ...
        if not self.command_queue.empty():
            ...
            if self.is_valid_command(command):
                ...
                if command_res.confidence <= float(
                    ...
                    self.try_to_recognize(repeated=True)
    ...
    def run(self):
        try:
            ...
            while True:
                try:
                    if not self.command_queue.empty():
                        transcript = self.command_queue.get()
